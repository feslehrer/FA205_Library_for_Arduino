...
void rs232_baud ( uint32_t baud )     // �ndert die Baudrate
{
    #define UART_UBRR_CALC(BAUD_,FREQ_) ((FREQ_)/((BAUD_)*16L)-1)
    #ifdef _ATMEGA8_
    UCSRB |= (1<<TXEN) | (1<<RXEN);    // UART TX und RX einschalten
...
{
  #ifdef _ATMEGA8_
   if (!(UCSRA & (1<<RXC))) return ('\0');    // Wenn kein Zeichen im Puffer, dann 0 zur�ckgeben!
   return UDR;                                // Zeichen zur�ckliefern
  #endif
  #ifdef _ATMEGA328_
   if (!(UCSR0A & (1<<RXC0))) return ('\0');    // Wenn kein Zeichen im Puffer, dann 0 zur�ckgeben!
   return UDR0;                                // Zeichen zur�ckliefern
  #endif
}
