/*
    ...
    Anmerkungen:		getestet mit Arduino-Carrier-Board und 
	                    Roboter-Motor-Shield (www.ase-schlierbach.de)
						und Conrad Raupenfahrwerk
*/
...
#define Input1  _PORTD_,2   // H-Brücken-Signale
#define Input2  _PORTB_,2
#define Input3  _PORTB_,4
#define Input4  _PORTB_,5
...
{ 
  ...
  // Motorsignale 
  bit_init(Input1,OUT);      // Input 1
  bit_init(Input2,OUT);      // Input 2 
  bit_init(Input3,OUT);      // Input 3
  bit_init(Input4,OUT);      // Input 4
...
void robby_richtung(uint8_t dir, uint8_t speed, int16_t fade)
{
  ...
  //Bewegungsrichtung
  bit_write(Input1,directions[dir][0]);       // Input 1
  bit_write(Input2,directions[dir][1]);       // Input 2
  bit_write(Input3,directions[dir][2]);       // Input 3
  bit_write(Input4,directions[dir][3]);       // Input 4
